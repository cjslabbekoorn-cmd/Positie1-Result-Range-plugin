 ...
 *
 * Expects a release asset like: listing-results-range-{version}.zip (or results-range-wpml-{version}.zip).
 */
class RRW_GitHub_Updater {
    ...
    protected static function find_asset_url( array $release ) : ?string {
        ...
        // Prefer assets that match configured prefix, but be tolerant to renamed zips.
        $prefixes = array_values( array_unique( array_filter( [
            (string) RRW_GH_ASSET_PREFIX,
            'listing-results-range-',
            'results-range-wpml-',
        ] ) ) );

        // 1) Strict match on known prefixes
        ...
        // 2) Fallback: if there's exactly one .zip asset, use it.
        $zip_urls = [];
        foreach ( $release['assets'] as $asset ) {
            if ( empty( $asset['name'] ) || empty( $asset['browser_download_url'] ) ) continue;
            ...
            if ( substr( $name, -4 ) === '.zip' ) {
                $zip_urls[] = (string) $asset['browser_download_url'];
            }
        }
